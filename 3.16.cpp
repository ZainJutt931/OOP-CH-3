...
{
public:
   HealthProfile(string fn,string ln,string g, int bd, int bm, int by, double h, double w ) {
      setFirstName( fn );
      setLastName( ln );
      setGender( g );
      setBirthDay( bd );
   ...
   void displayTargetHeartRate( int cd, int cm, int cy ) {
      cout << calculateMaximumHeartRate( cd, cm, cy ) * 0.5 << " - "<< calculateMaximumHeartRate( cd, cm, cy ) * 0.85;
   }
...
int main() {
   ...
   cin >> weight;
   HealthProfile healthProfile( fName, lName, gender, bDay, bMonth, bYear,height, weight );
   cout << "\nName: "
      << healthProfile.getFirstName() << ' '<< healthProfile.getLastName()<< "\nGender: "<< healthProfile.getGender()
      << "\nBirth date: "<< healthProfile.getBirthDay() << '.'<< healthProfile.getBirthMonth() << '.'<< healthProfile.getBirthYear()
      << "\nAge: "<< healthProfile.calculateAge( cDay, cMonth, cYear )<< " years\nHeight: "<< healthProfile.getHeight()
      << " inches\nWeight: "<< healthProfile.getWeight()<< " pounds\nMaximum heart rate: "
	  << healthProfile.calculateMaximumHeartRate( cDay, cMonth, cYear )<< " bpm\nTarget-heart-rate: ";
